...
post_install_config(){
  printf "# THIS FILE AHS BEEN AUTO GENERATED.\nDO NOT EDIT\n\n" >> "$HOME/.autozshrc"
  if [[ $1 == "htop" ]];then
    ...
    echo "alias man=batman" >> "$HOME/.autozshrc"
...
minimal_setup_linux(){
  sudo apt-get update
  sudo apt-get -yq install coreutils gcc vim curl zsh ripgrep
...
ubuntu_setup(){
  ...
  # for pkg in unzip curl zsh htop tmux fzf bat bat-extra exa autojump lazygit; do
